...
- (void)rotateWithAngle:(CGFloat)rotation
{
    [self handleGestureState:UIGestureRecognizerStateBegan];
    
    self.touchCenter = CGPointZero;

    CGPoint centerInView = CGPointMake(CGRectGetMidX(self.view.frame), CGRectGetMidX(self.view.frame)+40);

    self.rotationCenter = [self.imageView convertPoint:centerInView fromView:self.view];
    CGFloat deltaX = self.rotationCenter.x-self.imageView.bounds.size.width/2;
    CGFloat deltaY = self.rotationCenter.y-self.imageView.bounds.size.height/2;
    ...
    [self checkBoundsWithTransform:transform];
//    [self handleGestureState:UIGestureRecognizerStateEnded];
